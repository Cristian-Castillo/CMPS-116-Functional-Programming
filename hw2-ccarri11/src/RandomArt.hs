...
data Expr
  ...
  | ExpressionOperations Expr Expr Expr -- added the Expression Operations
  deriving (Show)
...
-- See A Data Type above added ExpressionsOperations added!
...
exprToString (Thresh e1 e2 e3 e4) = "("++exprToString e2++">"++exprToString e1++"?"++exprToString e3++":"++exprToString e4++")"
exprToString (ExpressionOperations e1 e2 e3) = "(cos(pi*("++exprToString e1++"*"++exprToString e2++"+"++exprToString e3++")))"
--------------------------------------------------------------------------------
...
eval x y (Thresh e1 e2 e3 e4) = if(eval x y e1 > eval x y e2) then eval x y e4 else eval x y e3
eval x y (ExpressionOperations e1 e2 e3) = cos(pi*(eval x y e1*eval x y e2+eval x y e3))
...
-- return exprssion and recurse build d-1
