{- | CSE 112 : Programming Assignment 2
...
assoc def key [] = def -- if empty list return def
assoc def key (x:xs)
  | (key == fst(x)) = snd(x)
  | otherwise = assoc def key xs
-- Pattern Match def key list
-- key being fst(x) which is a string "john", snd(x) being value associated with "John"
-- else recursively call assoc def key xs Note:xs is the 1+remainingOfArray
...
removeDuplicates ls = reverse (helper [] ls)
  where
    helper :: [Int] -> [Int] -> [Int]
    helper seen []     = seen -- base case 
    helper seen (x:xs) = helper seen' rest' -- 
      where
        seen'          = if(not(x `elem` seen)) then ([x] ++ seen) else seen
        rest'          = xs

-- idea(********):
-- entire list through rest and if we already seen something added to seen
-- other wise dont add to seem TIM TA LAB
...
wwhile :: (a -> (Bool, a)) -> a -> a
wwhile f x
  | (fst (f x) == True) = wwhile f (snd (f x))  -- apply fst of apply f on x, then do the operation
  | otherwise = snd (f x)                      -- return value after condition failed?

-- let f applied to x = pair (bool, val) summary of whats going on
--------------------------------------------------------------------------------
...
fixpointL f x = loop [] f x   
  where
    loop :: [Int] -> (Int -> Int) -> Int -> [Int]
    loop accu f x
      | x == f x     = accu ++ [x]  
      | otherwise    = loop (accu ++ [x]) f (f x) 
      
--  | x == f x       = [x]
--  | otherwise      = x : fixpointL f (f x)

-- followed pseudocode given, and append from class helped clarify
...
fixpointW f x = wwhile wwf x
 where
   wwf        = \x -> (x /= (f x),(f x))
-- only way i could think of to get another variable in
-- we were getting Int -> (bool, Int) error
-- wouldve been a lot easier if we could add param in usual syntax
